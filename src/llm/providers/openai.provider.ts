...
export class OpenAIProvider implements ILLMProvider {
  ...
  constructor(private readonly config: ConfigService<AppConfig, true>) {
    this.client = new OpenAI({
      apiKey: this.config.get('openai.apiKey', { infer: true }),
    });
    this.defaultModel = this.config.get('openai.defaultModel', { infer: true });
    this.embeddingModel = this.config.get('openai.embeddingModel', { infer: true });
  }
  ...
  async isAvailable(): Promise<boolean> {
    try {
      ...
      this.logger.warn('OpenAI provider unavailable');
      return false;
    }
  }
